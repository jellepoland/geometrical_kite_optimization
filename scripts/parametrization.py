...
class KiteDefinition():
    ...
    def get_arc(self, halve=False, plot=False):
        ...
        if halve:
            ...
            if len(LE_arc_y)//2 != 0:
                LE_coords = np.vstack([LE_coords, [LE_coords[-1,0],0,LE_coords[-1,2]]])
                TE_coords = np.vstack([TE_coords, [TE_coords[-1,0],0,LE_coords[-1,2]]])

        if plot:
            self.plot_arc(coords=[LE_arc_y, LE_arc_z, TE_arc_y, TE_arc_z], 
    ...
    def chord_vectors(self):
        ...
        for i, LE_data in enumerate(LE_coords):
            ...
            hchord_yz_offset = np.array([qchord[1], qchord[2]]) - np.array([LE_data[1], LE_data[2]])
            new_qchord = np.array([qchord[0], 
                                   LE_arc_proj_y[i] + hchord_yz_offset[0], 
                                   LE_arc_proj_z[i]] + hchord_yz_offset[1])
        ...
        full_arc_LE = np.vstack([new_LE_coords[:-1], np.flip(new_LE_coords[:-1],0)*np.array([1, -1, 1])])
        full_arc_TE = np.vstack([new_TE_coords[:-1], np.flip(new_TE_coords[:-1],0)*np.array([1, -1, 1])])
        full_arc_qchord = np.vstack([new_qchord_points[:-1], np.flip(new_qchord_points[:-1],0)*np.array([1, -1, 1])])

        return new_chord_vectors, np.array(new_qchord_points), new_LE_coords, new_TE_coords, full_arc_LE, full_arc_TE, full_arc_qchord
    ...
    def get_bezier_curve(self, normalized=False, plot=False):
        ...
        res = minimize(fit_error, x0=[delta0, gamma0], bounds=[(arc_y[-1], arc_y[0]), (arc_z[0], arc_z[-1])])
        ...
        if normalized:
            ...
            points = [np.array(point) - np.array([0, height]) for point in points] / self.projected_span
        ...
        if plot:
            ...
            plt.plot([points[i][0] for i in range(len(points))], [points[i][1] for i in range(len(points))], 'ro', label='Control Points')
            plt.scatter(LE_arc_proj_y, LE_arc_proj_z, color='blue', label='Projected LE Arc')
            plt.axis('equal')
            plt.xlabel('Y-axis')
            plt.ylabel('Z-axis')
        ...
        return bez_x, bez_y, np.array(points), LE_arc_proj_y, LE_arc_proj_z, phi, delta_best, gamma_best
...
class KiteScaling():
    ...
    def __init__(self, kitedefinition: KiteDefinition, new_ar, arc_parameters=[0.5398012, 0.42633213, 0.51579476]):
        if not isinstance(kitedefinition, KiteDefinition):
            raise TypeError("The kitedefinition input must be an instance of KiteDefinition")
    ...
    def plot_span_experiment(self, plot=False):
        ...
        _, _, old_points, LE_arc_proj_y, LE_arc_proj_z, _, _, _ = self.old_kite.get_bezier_curve(plot=False)
        ...
        if plot:
            ...
            normalized_y, normalized_z, control_norm = self.normalized_arc(LE_arc_proj_y, LE_arc_proj_z, control_points=old_points)
            ...
            plt.scatter([float(control[0]) for control in control_norm], [float(control[1]) for control in control_norm], color='blue', label='Normalized control points')
            print(control_norm)
            norm_y_new, norm_z_new = self.normalized_arc(np.array(new_y[::-1]), np.array(new_z[::-1]))
            plt.plot(norm_y_new, norm_z_new + (normalized_z[-1] - norm_z_new[-1]), 'm--', label='New Normalized Arc Data')
            ...
            plt.xlabel('Y-axis')
            plt.ylabel('Z-axis')
            plt.title(f'Effect of Span Scaling by {self.ar_scaling_factor:.2f} on the kite LE Arc')
            plt.legend()
            plt.grid()
            plt.show()
    ...
    def chord_vectors(self, plot=False):
        ...
        for i, LE_data in enumerate(LE_coords):
            ...
            old_chord_vector = TE_vector - LE_vector
            qchord = LE_vector + 0.25 * old_chord_vector
            hchord_yz_offset = np.array([qchord[1], qchord[2]]) - np.array([LE_vector[1], LE_vector[2]])
            new_qchord = np.array([qchord[0], 
        ...
        if plot:
            plt.plot(np.array(full_arc_LE)[:,1], np.array(full_arc_LE)[:,2], "g-", label='LE')
            plt.plot(np.array(full_arc_LE)[:,1], np.array(full_arc_LE)[:,2], "gx")
            plt.plot(np.array(full_arc_TE)[:,1], np.array(full_arc_TE)[:,2], "r-", label='TE')
            plt.plot(np.array(full_arc_TE)[:,1], np.array(full_arc_TE)[:,2], "rx")
            plt.plot(np.array(full_arc_qchord)[:,1], np.array(full_arc_qchord)[:,2], "b--", label='QC')
            plt.plot(np.array(full_arc_qchord)[:,1], np.array(full_arc_qchord)[:,2], "bx")
    ...
    def get_new_wing_sections(self):
        ...
        for i, LE_coord in enumerate(full_arc_LE):
            ...
            rib["VUP"] = self.config["wing_sections"]["data"][i][self.header_map["VUP_x"]:self.header_map["VUP_z"]+1]
            rib["airfoil_id"] = self.config["wing_sections"]["data"][i][self.header_map["airfoil_id"]]
    ...
    def export_data(self, filename="kite_data.txt"):
        ...
        new_yaml_file_path = Path(PROJECT_DIR) / "processed_data" / f"{self.kite_name}_scaled"
    ...
    def vsm_csv_sheets(self):
        ...
        new_csv_file_path = Path(PROJECT_DIR) / "processed_data" / f"{self.kite_name}_scaled"
        ...
        with open(bridle_csv_path, mode='w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow(['p1_x', 'p1_y', 'p1_z', 'p2_x', 'p2_y', 'p2_z', 'diameter'])
        ...
        for i, section in enumerate(wing_sections):
            ...
            d_tube = [airfoil_info[2]['meta_parameters']['d_tube_from_surfplan_txt'] for airfoil_info in airfoil_data if airfoil_info[0] == airfoil_id]
            camber = [airfoil_info[2]['meta_parameters']['eta'] for airfoil_info in airfoil_data if airfoil_info[0] == airfoil_id]
...
if __name__ == "__main__":
    ...
    scaled_kite.plot_arc([LE_full_y, LE_full_z, LE_y_scaled, LE_z_scaled_trans],
                         ["Old LE", "Scaled Kite"],
                         ["kx", "rx"],
                         "Scaled LE plot",
                         grid=True)
